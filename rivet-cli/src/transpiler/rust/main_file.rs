...
pub(super) struct MainParts {
    ...
    /// Whether any route declares a path parameter, so the crate root imports
    /// `Path` and no warning fires in a crate whose routes have none.
    pub(super) has_path_params: bool,
    /// Whether any route declares a query parameter, so the crate root
    /// imports `Query` only when a handler extracts it.
...
pub(super) fn assemble(blocks: &MainBlocks<'_>, parts: &MainParts) -> Result<String, Diagnostic> {
    ...
    let extract = match (parts.has_path_params, parts.has_query_params) {
        (true, true) => "Json, Path, Query, State",
        (true, false) => "Json, Path, State",
        (false, true) => "Json, Query, State",
        (false, false) => "Json, State",
    };
    Ok(format!(
        r#"// Generated by rivet {version}. Do not edit; run `rivet build` again.
use axum::{{extract::{{{extract}}}, routing::{{{routing}}}, Router}};
